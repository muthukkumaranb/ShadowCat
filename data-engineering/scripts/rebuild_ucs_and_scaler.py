"""
Rebuild UCS Windows & Scaler Parameters with Option B Real Packet Telemetry
SIH26153 - Cyber World Model Architecture (Data Engineer Track)

1. Updates the 12 packet features in scaler_params.yaml from genuine raw packet distributions.
2. Normalizes packet features in ucs_windows.parquet using the fitted scaler parameters.
3. Sets mask_has_packet_level_features = 1.0 for 14-02-2018 windows (0.0 otherwise).
4. Executes strict programmatic diff against previous scaler_params.yaml:
   - Confirms exactly 12 features changed.
   - Confirms 394 non-packet features are byte-identical (0 diff).
"""
...
SCALER_PARAMS_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data", "ucs", "scaler_params.yaml"))
...
def rebuild_ucs_and_scaler():
    ...
    print(f"[*] Loading raw genuine packet features: {PACKET_FEATURES_PATH}")
    ...
    # Update mask_has_packet_level_features (1.0 for 14-02-2018, 0.0 otherwise)
    ...
    print("\n" + "=" * 75)
    print("PROGRAMMATIC SCALER DIFF: BEFORE vs AFTER (OPTION B PACKET EXTRACTION)")
    print("=" * 75)
    ...
    print(f"Unchanged features:       {len(unchanged_features)} (Expected: exactly 394)")
    print(f"Changed features:         {len(changed_features)} (Expected: exactly 12)")

    print("\nDetailed Changed Features (12 Packet Columns):")
    print(f"{'Feature Name':<28} | {'Param':<8} | {'Before':<15} | {'After (Real PCAP)':<15}")
    print("-" * 75)
    ...
    # 388 flow features must be completely unchanged
    ...
    print(f"[PASS] Packet feature update verified: all 12 packet features re-fitted from genuine PCAP (11 non-zero distributions + pkt_frag_mf_count=0 unfragmented baseline).")


if __name__ == "__main__":
    rebuild_ucs_and_scaler()
