...
class ML2Trainer:
    ...
    def train(
        ...
        train_graphs = [train_dataset.get(i) for i in range(len(train_dataset))]
        val_graphs = [val_dataset.get(i) for i in range(len(val_dataset))]
        
        # Fit scaler on train strictly
        self.scaler = NodeFeatureScaler()
        self.scaler.fit(train_graphs)
        
        # Apply scaler
        scaled_train = self.scaler.transform(train_graphs)
        scaled_val = self.scaler.transform(val_graphs)
        
        # We need to wrap them back in SnapshotDataset for paired sequences
        from ml2.data.dataset import SnapshotDataset, build_paired_sequences
        scaled_train_dataset = SnapshotDataset(scaled_train)
        scaled_val_dataset = SnapshotDataset(scaled_val)

        train_pairs = build_paired_sequences(scaled_train_dataset)
        val_pairs = build_paired_sequences(scaled_val_dataset)
