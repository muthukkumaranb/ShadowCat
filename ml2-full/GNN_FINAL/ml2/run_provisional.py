...
def run_provisional_ablation(graphs, config):
    ...
    logger.info("\n" + "=" * 70)
    logger.info("PROVISIONAL ABLATION RESULTS")
    logger.info("=" * 70)
...
def main():
    ...
    # Task 2: Lateral-movement spot-check
    infiltration_edges_df = full_edges_df[full_edges_df["source_day"] == INFILTRATION_DAY]
    spotcheck_result = None
    ...
    ml1_cfg = config.get("ml1_interface", {})
    ml1_output_file = ml1_cfg.get("output_dir")
    adapter = ML1Adapter(ml1_output_file=ml1_output_file, mock_mode=False)
